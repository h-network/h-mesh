...
def _lifecycle_command(command: str, argv: list[str]) -> None:
    ...
    # ⚠ Fire-and-forget by design (see core.channels.send's own docstring),
    # for hire same as every other lifecycle command -- this prints the
    # stream_id once the envelope is durably enqueued (ADMITTED), never a
    # word implying the agent actually exists (CREATED). A caller that
    # prints "hired" off nothing but a clean exit here is making that
    # exact mistake -- see setup.sh's roster-hire loop for the real
    # incident this corrected. A previous version of this command offered
    # --wait to poll for stronger evidence; removed (not fixed) because
    # exit 0 was never reachable -- nothing in this codebase ties a
    # *successful* StartAgent back to the specific stream_id that caused
    # it, so every call still returned "unknown" after burning up to 30s.
    # See held ticket 74f1edf3 for the attributable-completion signal a
