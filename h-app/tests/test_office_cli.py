...
@patch("modules.office.cli.send")
def test_hire_prints_the_stream_id_and_returns_immediately(mock_send, monkeypatch, capsys):
    # hire is fire-and-forget, full stop -- there used to be a --wait flag
    # offering to poll for stronger evidence; removed (not fixed) because
    # its exit 0 was never reachable, so every call paid up to 30s to
    # learn "unknown". Bare hire prints the stream_id (ADMITTED) and
    # returns immediately; it never claims the agent exists (CREATED).
    _env(monkeypatch)
    mock_send.return_value = "stream-1"
    r = FakeRedis()
    with patch("modules.office.cli._context", return_value=(r, POD, TENANT, "architect")):
